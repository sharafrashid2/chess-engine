...
def mini_max(tree, deepest_level):
    ...
    def traverse(tree, depth, turn, alpha, beta):
        ...
        for key in tree:
            ...
            if turn == 'black' and score >= beta:
                return beta
            elif turn == 'black' and score > alpha:
                alpha = score
                if depth == 0:
                    if len(start) > 0:
                        start.pop(0)
                    start.append(key)
            elif turn == 'white' and score <= alpha:
                return alpha
            elif turn == 'white' and score < beta:
                beta = score
                if depth == 0:
                    if len(start) > 0:
                        start.pop(0)
        ...
        return alpha if turn == 'black' else beta

    total = traverse(tree, 0, 'black', float('-inf'), float('inf'))

    return None if not start else start[0], total
